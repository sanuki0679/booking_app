<x-app-layout>
    <div class="shadow bg-white border rounded w-full max-w-md mx-auto mt-10">
        <h1 class="text-3xl text-center font-semibold p-6">{{ __('Booking Form') }}</h1>

        <x-validation-errors class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mx-6" />

        <form action="{{ route('bookings.store') }}" method="POST" class="relative px-6 pb-6 flex-auto">
            @csrf
            ...
            <div class="my-4 text-slate-500 text-lg leading-relaxed">
                ...
                <select name="booking_type" id="booking_type" required
                    ...
                    <option value="席のみ後日連絡">席のみ後日連絡</option>
                    <option value="和コース">和コース</option>
                    <option value="彩々コース">彩々コース</option>
                    <option value="煌コース">煌コース</option>
                </select>
            </div>
            <div class="my-4 text-slate-500 text-lg leading-relaxed mb-2">
                ...
                <textarea name="body" id="body" placeholder="{{ __('Description') }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline h-32">{{ old('booking_details') }}</textarea>
            </div>
            <div class="my-4 text-slate-500 text-lg leading-relaxed">
